...
上面的「手动滚动祖先链」只处理一种情况：用户从分组标题行开始滑动，还没进入真实拖拽。
...
```js
handle: '.drag-handle, .group-drag-surface'
```

含义是：

- 预设条目只能从 `.drag-handle` 开始拖。
- 分组标题行可以从 `.group-drag-surface` 开始拖，但它走长按，不走秒拖。
- 按钮、输入框、菜单等交互区域继续由 `filter` 排除。

当用户开启“手机预设条目整条拖拽”时，可以去掉 `handle` 限制，让条目非按钮区域也能作为拖拽起点。但这时仍要保留动态 delay：
...
| 手机端从标题行滑动 | 标题行 `touch-action: none`，长按前识别纵向滑动并手动滚动祖先链 | 手动滚动祖先链 |
| 手机端标题行长按拖分组 | 320ms ready feedback，之后由 Sortable 进入 `onStart` | ready feedback 设计 |
...
15. 手机端条目把手允许 delay 0；条目整行和分组标题行仍然使用长按 delay。
